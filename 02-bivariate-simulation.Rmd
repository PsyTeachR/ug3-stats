...
```{r pairs, fig.height=6, fig.width=6, out.width=400, out.height=400, fig.cap="Pairwise correlations for the starwars dataset"}
...
```{r massive-creature, fig.height=6, fig.width=6, out.width=400, out.height=400, fig.cap="Pairwise correlations for the starwars dataset after removing outlying mass value."}
starwars2 <- starwars %>%
...
```{r bye-yoda, fig.height=6, fig.width=6, out.width=400, out.height=400, fig.cap="Pairwise correlations for the starwars dataset after removing outlying mass and birth_year values."}
starwars3 <- starwars2 %>%
  filter(name != "Yoda")
...
knitr::include_app("https://shiny.psy.gla.ac.uk/Dale/bivariate", height="410px")
```
...
The $\beta_1$ value tells you that for each change in $X$ you have a corresponding change of `r b1` for $Y$, and you know that the line goes through the points $(\bar{X}, \bar{Y})$ as well as the y-intercept $(0, \beta_0)$. 

Think about stepping back unit-by-unit from $X = \bar{X}$ to $X = 0$. 
At $X = \bar{X}$, $Y = `r .mw`$. Each unit step you take backward in the X dimension, $Y$ will drop by $\beta_1 = `r b1`$ units.  When you get to zero, $Y$ will have dropped from $\bar{Y}$ to $\bar{Y} - \bar{X}\beta_1$.

So the general solution is: $\beta_0 = \bar{Y} - \bar{X}\beta_1$.

Since $\beta_1 = `r b1`$, $\bar{X} = `r .mh`$, and $\bar{Y} = `r .mw`$, $\beta_0 = `r b0 <- .mw - .mh * b1; b0`$. Thus, our regression equation is:
