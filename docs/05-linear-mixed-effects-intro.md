...
Before proceeding, there is one important thing to note. Unfortunately, the description of the data in the `sleepstudy` package is inaccurate. Specifically, it states:
...
In short, there were seven days of sleep restriction, not ten. The days coded as `0` and `1` were adaptation and training, and the day coded as `2`, where the baseline measurement was taken should be the place where we start measuring.
...
Partial pooling is what happens when you treat a factor as random: estimates at each level of the factor (i.e., for each subject) become informed by information from other levels (i.e., other subjects).
